...
The system SHALL allow at most one active schedule per `(patient_id, order_id)` pair. Two different orders for the same product are allowed. Enforcement happens both in `CreateScheduleValidator` (pre-transaction lookup) and at the database level via a partial unique index `(patient_id, order_id) WHERE order_id IS NOT NULL`, which is the authoritative race guard.

#### Scenario: Patient already has a schedule for the given order
- **WHEN** the patient POSTs a schedule with an `order_id` for which they already own a schedule
...
- **THEN** the database partial unique index SHALL make one of them fail and the second response SHALL be a `409`
